...
DEPEND="dev-python/beautifulsoup:python-2
	...
	>=dev-libs/protobuf-2.5.0[python,python_targets_python2_7?]
	dev-python/notify2[python_targets_python2_7?]
...
src_install() {
	if use systemd ; then
		...
		doins "${FILESDIR}/pulseaudio-dlna.service"
	fi

	PREFIX=/usr emake install DESTDIR="${D}" || die
}
